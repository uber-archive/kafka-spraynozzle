...
.PHONY: download extract build-kafka build run clean
...
build: extract build-kafka KafkaSpraynozzle.class

KafkaSpraynozzle.class:
	$(JC) -cp $(CLASSPATH) kafka-spraynozzle.java

run: KafkaSpraynozzle.class
	$(JAVA) -cp $(CLASSPATH) KafkaSpraynozzle
